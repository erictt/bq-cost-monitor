...
`;
   ...
   */
  static addCostEstimation(query, options = {}) {
    const { 
      estimatedTB = 'X', 
      costPerTB = 5 
    } = options;
...
`;
   ...
   */
  static addQueryTagging(query, projectId, teamName, options = {}) {
    const { owner = '', purpose = '' } = options;
    
    let tagging = `
-- Query for project: ${projectId}
-- Team: ${teamName}
-- Generated by bq-cost-monitor
`;
...
`;
   ...
   */
  static addPartitionFilter(query, partitionColumn, partitionFilter) {
   ...
   */
  static makeIncremental(query, incrementalColumn, incrementalPredicate) {
    return `
-- Incremental build query
-- Only processes new/changed data since last build
${query.trim()}
${QueryOptimization.getIncrementalWhereClause(incrementalColumn, incrementalPredicate)}
`;
  }
   ...
   */
  static getIncrementalWhereClause(incrementalColumn, incrementalPredicate) {
    return `
...
`;
   ...
   */
  static addExplorationLimit(query, limit = 1000) {
   ...
   */
  static addCostMonitoring(query, options = {}) {
    const { 
      queryId = 'query_id', 
      queryType = 'adhoc' 
    } = options;
